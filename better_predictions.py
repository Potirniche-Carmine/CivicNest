...
def employment_stats_by_zip(conn, cur):
    try:
        ...
        print(f"[ERROR] Failed to fetch employment stats: {e}")
...
def cluster_dist_by_zip(conn, cur):
    try:
        ...
        print(f"[ERROR] Cluster dist query failed: {e}")
...
def employment_by_cluster(conn, cur):
    try:
        ...
        print(f"[ERROR] employment_by_cluster failed: {e}")
...
def payroll_price_ratio(conn, cur):
    try:
        ...
        return rows  # Return for future use
    except Exception as e:
        print(f"[ERROR] payroll_price_ratio failed: {e}")
        return []
...
def predicted_growth(conn, cur):
    try:
        ...
        print(f"[ERROR] predicted_growth failed: {e}")
...
def growth_vs_others(conn, cur):
    try:
        ...
        print(f"[ERROR] growth_vs_others failed: {e}")
...
def update_insights(conn, cur, cluster_vals):
    try:
        ...
        for cid, payroll, price, ratio in cluster_vals:
            cur.execute("""
                INSERT INTO insights_table (cluster_id, avg_payroll, avg_price, affordability_ratio)
                VALUES (%s, %s, %s, %s)
            """, (cid, payroll, price, ratio))

        conn.commit()
        print("insights_table updated.")
    except Exception as e:
        print(f"[ERROR] Failed to update insights table: {e}")
...
def better_predictions():
    ...
    predicted_growth(conn, cur)
    growth_vs_others(conn, cur)

    if ratio_data:
        update_insights(conn, cur, ratio_data)
